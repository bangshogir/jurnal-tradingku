//+------------------------------------------------------------------+
//|                                                 RiskPanel.mq5    |
//|            Risk Manager Panel + Cut Loss (Expert Advisor)        |
...
void CRiskPanel::UpdateStats()
  {
   ...
   // Kalkulasi Daily ATR (14 hari terakhir) secara manual
   double atr_sum = 0;
   for(int i=1; i<=14; i++)
...
void CRiskPanel::OnPlace()
  {
   ...
   // Kompensasi Spread (Entry Tetap, SL Dilebarkan)
   double sl_orig = sl;
   if(is_buy)
     sl = NormalizeDouble(sl - spread, digits);  // Buy: SL turun makin jauh
   else
     sl = NormalizeDouble(sl + spread, digits);  // Sell: SL naik makin jauh
   ...
   if(m_cl_active)
     {
      // CUT LOSS MODE: backup SL = 2x jarak, cut level disimpan di comment
...
bool CRiskPanel::Create(const long chart,const string name,const int sw,
                        const int x1,const int y1,const int x2,const int y2)
  {
   ...
   m_btn_place.ColorBackground(C'30,144,255'); m_btn_place.Color(clrWhite); // DodgerBlue
...
int OnInit()
  {
   if(!ExtPanel.Create(0,"Risk Panel",0,20,30,300,410)) return INIT_FAILED;
   ExtPanel.Run();
