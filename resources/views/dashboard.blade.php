    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 overflow-hidden">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-1 bg-slate-50 border border-slate-200 rounded-lg p-1 shadow-sm">
                    <a href="{{ route('dashboard', ['month' => $prevMonth->month, 'year' => $prevMonth->year, 'sort' => $sort, 'filter' => $filter, 'date_filter' => $dateFilter]) }}" 
                    ...
                    <a href="{{ route('dashboard', ['month' => $nextMonth->month, 'year' => $nextMonth->year, 'sort' => $sort, 'filter' => $filter, 'date_filter' => $dateFilter]) }}" 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
            <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h3 class="text-lg font-bold text-slate-900 tracking-tight">Statistik Pair</h3>
            ...
            <div class="flex-1 overflow-auto max-h-[400px]">
                <table class="w-full text-left border-collapse">
                    <thead class="sticky top-0 bg-slate-50/95 backdrop-blur z-10">
                        <tr class="border-b border-slate-100 text-[11px] uppercase tracking-wider text-slate-500 font-semibold">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($pairStats as $stat)
                            @php
                                $totalTradesPerPair = $stat->wins + $stat->losses;
                                $pairWinRate = $totalTradesPerPair > 0 ? round(($stat->wins / $totalTradesPerPair) * 100) : 0;
                            @endphp
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="p-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <span class="text-xs font-bold text-slate-700 w-8">{{ $pairWinRate }}%</span>
                                        <div class="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                            <div class="h-full bg-brand-500 rounded-full" style="width: {{ $pairWinRate }}%"></div>
                                        </div>
                                    </div>
                                </td>
